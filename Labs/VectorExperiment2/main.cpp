...
int main() {
    ...
    won = false;
    code = getRandomCode();
    currentGuess = {0, 0, 1, 1}; //0011

    //Create the set of 10000 possible codes
    createSet();
    ...
    cout << endl;
    cout << "Set Length: " << combinations.size() << endl;
    cout << "First in set: ";
    ...
    while (!won) {
        ...
        //cout << "Set Length after removing: " << combinations.size() << endl;
        
        removeCode(candidateSolutions, currentGuess);
        //cout << "Candid. Sol Len.: " << candidateSolutions.size() << endl;
...
vector<int> getNextGuess(vector<vector<int>> nextGuesses) {
    ...
    for (int i = 0; i < nextGuesses.size(); ++i) {
        if (find(candidateSolutions.begin(), candidateSolutions.end(), nextGuesses[i]) != candidateSolutions.end()) {
            cout << "Return from candidate..." << endl;
    ...
    for (int j = 0; j < nextGuesses.size(); ++j) {
        if (find(combinations.begin(), combinations.end(), nextGuesses[j]) != combinations.end()) {
            cout << "Return from combinations..." << endl;
